...
# 4. Pre-generation (one-time, ~$0 on Voyage free tier)
uv run python -m capgroup.pregen.style_audit
uv run python -m capgroup.pregen.disclosure_urls
uv run python -m capgroup.pregen.fetch_articles
uv run python -m capgroup.pregen.embed

# 5. Generate posts for all 16 test articles, 4 variants each, with flavor tags
