...
class HoldoutCluster:
    ...
    audience_track: str
    title: str
    real_posts: list[dict] = field(
        default_factory=list
    )  # list of {"postId", "text_clean", "embedding": np.ndarray}
...
def embed_via_voyage(
    ...
    for i in range(0, len(texts), BATCH):
        res = client.embed(
            texts=texts[i : i + BATCH], model=model, input_type=input_type
        )
...
def compute_metrics_for_gen_post(gen_vec: np.ndarray, cluster: HoldoutCluster) -> dict:
    ...
    return {
        ...
        "gen_vs_best_real_postId": int(
            cluster.real_posts[int(sims.argmax())]["postId"]
        ),
...
def run_holdout_eval(
    ...
    for cluster, query_vec in zip(clusters, title_query_vecs):
        ...
        for variant_idx, flavor in enumerate(flavors_for_variants, start=1):
            ...
            print(
                f"    v{variant_idx} [{attempt.flavor or '-':<14}] "
    ...
    gen_vecs = embed_via_voyage(
        voyage_client, [a.post for a in gen_attempts], "document", embed_model
    )
    ...
    real_judgments: dict[
        int, dict
    ] = {}  # postId -> {"voice_match", "on_topic", "hook_strength", "compliance", "notes"}
    for cluster in clusters:
        ...
        for rp in cluster.real_posts:
            ...
            print(
                f"  real postId={rp['postId']} ({cluster.audience_track}): "
    ...
    multi_paraphrase_ceiling = (
        np.mean([v for v in ceilings.values() if v is not None])
        if any(v is not None for v in ceilings.values())
        else None
    )
    ...
    real_voice = avg(
        [r["voice_match"] for rs in real_judges_by_url.values() for r in rs]
    )
    real_topic = avg([r["on_topic"] for rs in real_judges_by_url.values() for r in rs])
    real_hook = avg(
        [r["hook_strength"] for rs in real_judges_by_url.values() for r in rs]
    )
    real_compl_fail = sum(
        1 for rs in real_judges_by_url.values() for r in rs if r["compliance"] == "fail"
    )
    gen_compl_fail = sum(1 for row in rows if row["compliance"] == "fail")
    real_valid_fail = sum(
        1
        for rs in real_judges_by_url.values()
        for r in rs
        if r["is_valid_post"] == "fail"
    )
    gen_valid_fail = sum(1 for row in rows if row["is_valid_post"] == "fail")
    n_real = sum(len(rs) for rs in real_judges_by_url.values())

    md = [
        f"# Held-out evaluation — {datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')}",
        "",
    ]
    ...
    md.append(
        f"- Gen model: `{gen_model}`  /  Judge model: `{judge_model}`  /  Embed model: `{embed_model}`"
    )
    md.append("")
    md.append("## Embedding-based similarity")
    md.append(
        f"- **gen_vs_best_real** (avg across all gen posts): **{df['gen_vs_best_real'].mean():.3f}**"
    )
    md.append(f"- **gen_vs_centroid** (avg): {df['gen_vs_centroid'].mean():.3f}")
    if multi_paraphrase_ceiling is not None:
        md.append(
            f"- **inter-human ceiling** (avg pairwise cosine within multi-paraphrase clusters): **{multi_paraphrase_ceiling:.3f}**"
        )
    ...
    md.append("|---------------|-----------------|---------------------|-------|")
    md.append(
        f"| voice_match   | {gen_voice:.2f}            | {real_voice:.2f}                | {gen_voice - real_voice:+.2f} |"
    )
    md.append(
        f"| on_topic      | {gen_topic:.2f}            | {real_topic:.2f}                | {gen_topic - real_topic:+.2f} |"
    )
    md.append(
        f"| hook_strength | {gen_hook:.2f}            | {real_hook:.2f}                | {gen_hook - real_hook:+.2f} |"
    )
    md.append(
        f"| compliance fails | {gen_compl_fail}/{len(rows)} | {real_compl_fail}/{n_real} | — |"
    )
    md.append(
        f"| is_valid_post fails | {gen_valid_fail}/{len(rows)} | {real_valid_fail}/{n_real} | — |"
    )
    md.append("")
    ...
    for c in clusters:
        ...
        md.append(
            f"| `{urlslug}` | {c.audience_track} | {len(c.real_posts)} | {avg_best:.3f} | {ceiling_str} |"
        )
    md.append("")
