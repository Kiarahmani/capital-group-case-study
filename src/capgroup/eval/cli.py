...
from capgroup.eval.holdout import (
    REPO_ROOT,
    TRAIN_XLSX,
    run_holdout_eval,
    sample_holdout_urls,
...
def evaluate(
    ...
    min_paraphrases: int = typer.Option(
        3,
        "--min-paraphrases",
        help="Bias sampling toward URLs with at least this many real posts.",
    ),
    seed: int = typer.Option(2026, "--seed"),
    variants: int = typer.Option(4, "--variants"),
    flavors: Optional[str] = typer.Option(
        "all", "--flavors", help="Same syntax as capgroup-gen; default 'all'."
    ),
    ...
    holdout_urls = sample_holdout_urls(
        train_df, tracks, per_track, min_paraphrases, seed
    )
